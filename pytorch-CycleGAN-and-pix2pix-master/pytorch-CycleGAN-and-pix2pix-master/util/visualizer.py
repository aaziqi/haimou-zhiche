...
class Visualizer:
    ...
    def __init__(self, opt):
        ...
        self.name = opt.name
        self.saved = False
        self.use_wandb = opt.use_wandb and wandb is not None
    ...
    def log_epoch_time(self, epoch: int, total_epochs: int, seconds: float):
        local_rank = int(os.environ.get("LOCAL_RANK", 0))
        if local_rank != 0:
            return
        message = f"End of epoch {epoch} / {total_epochs} \t Time Taken: {seconds:.0f} sec\n"
        with open(self.log_name, "a") as log_file:
