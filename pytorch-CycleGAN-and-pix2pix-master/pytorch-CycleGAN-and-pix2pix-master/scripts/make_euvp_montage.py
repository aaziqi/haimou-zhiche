...
def find_real_fake_for_id(pred_dir: Path, img_id: int):
    candidates = [
        ...
        (
            pred_dir / f"{img_id}_real.png",
            pred_dir / f"{img_id}_fake.png",
        ),
        (
            pred_dir / f"{img_id}__real.png",
            pred_dir / f"{img_id}__fake.png",
        ),
    ]
...
def merge_case_rows(grid, cases_per_row: int):
    if cases_per_row <= 1 or len(grid) <= 1:
        return grid
    merged = []
    step = max(1, int(cases_per_row))
    for start in range(0, len(grid), step):
        row = []
        for part in grid[start:start + step]:
            row.extend(part)
        merged.append(row)
    return merged
...
def main():
    ...
    parser.add_argument("--cases_per_row", type=int, default=1)
    ...
    if args.mode == "cases":
        ...
        grid = merge_case_rows(grid, args.cases_per_row)
